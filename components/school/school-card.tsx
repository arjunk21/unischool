...
const BOARD_LABEL: Record<string, string> = {
  CBSE: 'CBSE', ICSE: 'ICSE', IB: 'IB', CAMBRIDGE: 'Cambridge', STATE_BOARD: 'State Board',
}
const TYPE_LABEL: Record<string, string> = {
  K12: 'K–12', PRIMARY: 'Primary', SECONDARY: 'Secondary',
  SENIOR_SECONDARY: 'Sr. Secondary', COLLEGE: 'College', COACHING: 'Coaching',
}
...
export function SchoolCard({ s }: { s: CardSchool }) {
  return (
    <Link href={`/schools/${s.slug}`}
      className="card flex flex-col overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer">
      {/* Cover */}
      ...
      <div className="p-4 pt-8 flex flex-col flex-1">
        ...
        <div className="flex flex-wrap gap-1 mb-3">
          ...
          {s.boards.slice(0,2).map(b => (
            <span key={b} className="badge-gray">{BOARD_LABEL[b] ?? b}</span>
          ))}
