  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
      <div className="flex items-center gap-4 mb-2">
        <h1 className="text-2xl font-extrabold text-gray-900">Admin — Claim Requests</h1>
        <a href="/admin/enquiries" className="btn-outline text-sm py-1.5 px-4">View All Enquiries →</a>
      </div>
      <p className="text-gray-500 text-sm mb-8">Approve or reject school profile claims.</p>

      <div className="grid grid-cols-3 gap-4 mb-8">
        {[['Pending',claims.filter(c=>c.status==='PENDING').length,'text-accent'],['Approved',claims.filter(c=>c.status==='APPROVED').length,'text-teal'],['Rejected',claims.filter(c=>c.status==='REJECTED').length,'text-red-500']].map(([l,v,c])=>(
          <div key={String(l)} className="card p-4 text-center"><div className={`text-2xl font-extrabold ${c}`}>{v}</div><div className="text-xs text-gray-500 mt-1">{l}</div></div>
        ))}
      </div>
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="text-sm text-gray-600 space-y-0.5">
                  ...
                  <p><span className="font-medium">User:</span> {c.user.name} ({c.user.email})</p>
                  <p><span className="font-medium">Official email:</span> {c.officialEmail} | {c.phone}</p>
                  {c.message && <p className="text-gray-500 italic">"{c.message}"</p>}
